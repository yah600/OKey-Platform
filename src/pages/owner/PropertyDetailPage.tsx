...
export default function PropertyDetailPage() {
  ...
  const units = getUnitsByProperty(id || '');
  const maintenanceRequests = allMaintenanceRequests
    .filter((req) => req.propertyId === id)
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 text-sm text-neutral-600">
                ...
                <span>{property.address.street}, {property.address.city}, {property.address.province} {property.address.postalCode}</span>
              </div>
            </div>
          </div>
              ...
              <Card>
                ...
                <div className="space-y-3 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-neutral-900 capitalize">{property.type}</span>
                  </div>
                  <div className="flex justify-between">
              ...
              {units.map((unit) => (
                <Card key={unit.id} className="hover:border-neutral-300 transition-colors cursor-pointer">
                  <div className="flex items-start justify-between mb-3">
                    <h4 className="font-semibold text-neutral-900">Unit {unit.unitNumber}</h4>
                    <Badge
                      variant={
                  ...
                  <div className="pt-3 border-t border-neutral-100 flex items-center justify-between">
                    <span className="text-lg font-semibold text-neutral-900">${unit.rent.toLocaleString()}/mo</span>
                    {unit.tenantName && (
                      <span className="text-xs text-neutral-600">{unit.tenantName}</span>
                    )}
                  </div>
                </Card>
              ))}
        ...
        {activeTab === 'maintenance' && (
          <div className="space-y-3">
            {maintenanceRequests.length === 0 ? (
              <Card>
                <p className="text-sm text-neutral-600 text-center py-8">No maintenance requests for this property.</p>
              </Card>
            ) : (
                ...
                <Card key={request.id}>
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-sm text-neutral-600 mb-1">Unit {request.unitNumber}</p>
                      <p className="text-xs text-neutral-500">{request.description}</p>
                    </div>
                    <div className="flex items-center gap-2">
                      <Badge
                        variant={
